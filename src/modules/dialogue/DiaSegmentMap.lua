...
-- New function to clean format characters from text
local function clean_format_characters(text)
	local cleanedText = text

	-- Remove all pipe characters
	cleanedText = cleanedText:gsub("|", "")

	-- Remove underscore formatting (but keep the content between underscores)
	cleanedText = cleanedText:gsub("_(.-)_", "%1")
...
do
	...
	function CharMapRenderer:set_original_text(text)
		-- Clean format characters from the original text
		self.original_text = clean_format_characters(text)
		self.label.Text = ""
	end
...
function DiaSegmentMap.StepAndRender(text: string, label: TextLabel, config)
	...
	-- Clean the text for final display but keep original for processing
	local cleaned_text = clean_format_characters(text)
	renderer:set_original_text(cleaned_text)
	...
	for _, segment in ipairs(text_formatted) do
		...
		-- Clean format characters from the segment text
		local clean_seg_text = clean_format_characters(seg_text)
		...
		if segment.skip then
			...
			visible_text = visible_text .. clean_seg_text
