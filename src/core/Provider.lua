...
local function wait_for_game_loaded()
...
local function extract_content_urls(instance)
	local urls = {}
	...
	for _, prop in ipairs(properties) do
		...
		if success and result and typeof(result) == "string" and result:match("^rbxasset://") or result:match("^rbxassetid://") then
			...
			if not processed_assets[assetKey] then
				table.insert(urls, result)
				processed_assets[assetKey] = true
			end
		end
	end
...
local function collect_assets(container: Instance)
	local assets = {}

	for _, child in ipairs(container:GetChildren()) do
		local urls = extract_content_urls(child)
		if urls then
		...
		if child:IsA("Folder") or child:IsA("Model") then
			local childAssets = collect_assets(child)
...
function Provider.PreloadAsyncDescendants(parent: Instance)
	...
	for _, asset in ipairs(collect_assets(parent)) do
		table.insert(assets, asset)
	end
